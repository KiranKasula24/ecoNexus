...
export default function SetupPage() {
  ...
  const checkSetupStatus = async () => {
    ...
    // Check if setup is already complete based on entity type
    let isComplete = false;

    if (company.entity_type === "manufacturer") {
      // Check if they have materials or requirements
      ...
      isComplete = !!materials && materials.length > 0;
    } else if (company.entity_type === "recycler") {
      ...
      isComplete = profile?.is_setup_complete || false;
    } else if (company.entity_type === "processor") {
      const { data: profile } = await supabase
      ...
      isComplete = profile?.is_setup_complete || false;
    } else if (company.entity_type === "logistics") {
      const { data: profile } = await supabase
    ...
    // Redirect to dashboard if setup complete
    if (isComplete) {
...
function ManufacturerSetup() {
  ...
  const handleComplete = async () => {
    // Mark setup as complete (can add a field in companies table if needed)
    ...
    <div className="bg-white shadow rounded-lg p-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Let's set up your manufacturing profile and deploy your Nexa agent
        </p>
      </div>
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            <h3 className="font-semibold text-blue-900 mb-2">What is Nexa?</h3>
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          processing_capacity_tons_month: parseFloat(
            formData.processing_capacity,
          ),
          processing_methods: formData.processing_methods,
          output_quality_tiers: formData.output_quality,
          min_pickup_volume_tons: parseFloat(formData.min_pickup_volume),
          max_contamination_tolerance: parseFloat(formData.max_contamination),
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {formData.accepted_materials.map((material) => (
              <div key={material} className="flex items-center space-x-3 mb-3">
  ...
  const handleComplete = async () => {
    try {
      const { error } = await supabase
        ...
        .update({
          fleet_capacity: {
            trucks: parseInt(formData.fleet_trucks),
            total_capacity_tons: parseFloat(formData.fleet_capacity),
          },
          vehicle_types: formData.vehicle_types,
          service_regions: formData.service_regions,
          max_distance_km: parseInt(formData.max_distance),
          material_specializations: formData.material_specializations,
          base_rate_per_ton_km: parseFloat(formData.base_rate),
          minimum_load_tons: parseFloat(formData.min_load),
          consolidation_discount_percentage: parseFloat(
            formData.consolidation_discount,
          ),
        ...
        <div className="space-y-6">
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-6">
            ...
            <ul className="text-sm text-orange-800 space-y-1 list-disc list-inside">
              ...
              <li>
                Identify consolidation opportunities (many-to-one, one-to-many)
              </li>
